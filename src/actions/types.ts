...
export const schemaToUsage = (schema: z.ZodObject<z.ZodRawShape>): string => {
  ...
  for (const [key, value] of Object.entries(shape)) {
    ...
    if (key === "action") {
      ...
      // 数组用 [] 表示，字符串用 "xxx" 表示
      const formatted = Array.isArray(example)
...
const zodToExample = (schema: z.ZodTypeAny): unknown => {
  ...
  const type = (s._zod as Record<string, unknown>)?.def?.type || s.type;
  ...
  switch (type) {
    ...
    case "tuple":
      ...
      return (
        s._zod?.def?.items?.map((item: z.ZodTypeAny) => zodToExample(item)) ??
        []
      );
    default:
      return "?";
