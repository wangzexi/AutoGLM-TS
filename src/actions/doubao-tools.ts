...
export const DOUBAO_TOOLS: ToolDefinition[] = [
  // 1. Launch - 启动应用
  {
    ...
    name: "launch_app",
    description:
      '启动指定的应用。参数：app - 应用包名（如 "com.tencent.mm" 表示微信）',
    parameters: (() => {
  ...
  // 2. Tap - 点击屏幕
  {
    ...
    name: "tap_screen",
    description:
      "点击屏幕指定位置。坐标范围 0-1000，原点左上角。参数：x, y - 点击坐标",
    parameters: (() => {
      const schema = z.object({
  ...
  // 4. Type_Name - 输入人名
  {
    ...
    name: "type_name",
    description:
      "输入人名的操作，基本功能同 type_text。参数：text - 要输入的人名",
    parameters: (() => {
      const schema = z.object({
  ...
  // 5. Interact - 交互
  {
    ...
    name: "interact",
    description:
      "当有多个满足条件的选项时而触发的交互操作，询问用户如何选择。无参数。",
    parameters: (() => {
      const schema = z.object({});
      const jsonSchema = schema.toJSONSchema();
  ...
  // 6. Swipe - 滑动
  {
    ...
    name: "swipe_screen",
    description:
      "从起始坐标滑动到结束坐标。参数：start_x, start_y - 起始坐标；end_x, end_y - 结束坐标。坐标范围 0-1000",
    parameters: (() => {
      const schema = z.object({
        start_x: z
          .number()
          .min(0)
          .max(1000)
          .describe("起始点 x 坐标，范围 0-1000"),
        start_y: z
          .number()
          .min(0)
          .max(1000)
          .describe("起始点 y 坐标，范围 0-1000"),
        end_x: z
          .number()
          .min(0)
          .max(1000)
          .describe("结束点 x 坐标，范围 0-1000"),
        end_y: z
          .number()
          .min(0)
          .max(1000)
          .describe("结束点 y 坐标，范围 0-1000"),
      });
      const jsonSchema = schema.toJSONSchema();
  ...
  // 9. Back - 返回
  {
    ...
    name: "back",
    description:
      "导航返回到上一个屏幕或关闭当前对话框。相当于按下 Android 的返回按钮。无参数。",
    parameters: (() => {
      const schema = z.object({});
      const jsonSchema = schema.toJSONSchema();
  ...
  // 12. Take_over - 接管
  {
    ...
    name: "take_over",
    description:
      "表示在登录和验证阶段需要用户协助。参数：message - 接管提示信息",
    parameters: (() => {
      const schema = z.object({
  ...
  // 14. Call_API - 调用 API
  {
    ...
    name: "call_api",
    description:
      "总结或评论当前页面或已记录的内容。参数：instruction - API 调用指令",
    parameters: (() => {
      const schema = z.object({
