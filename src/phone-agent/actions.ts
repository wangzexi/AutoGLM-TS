...
export class ActionHandler {
  ...
  async execute(
    ...
    try {
      return await handler.call(this, action, screenWidth, screenHeight);
    } catch (error) {
      return {
        success: false,
        shouldFinish: false,
  ...
  private getHandler(
    actionName?: string
  ): ((action: Record<string, unknown>, width: number, height: number) => Promise<ActionResult>) | null {
    const handlers: Record<
      string,
      (action: Record<string, unknown>, width: number, height: number) => Promise<ActionResult>
    > = {
  ...
  private async handleLaunch(
    ...
    const success = await launchApp(appName, this.deviceId);
    if (success) {
  ...
  private async handleType(
    ...
    await typeText(text, this.deviceId);
    await new Promise((resolve) => setTimeout(resolve, 1000));
  ...
  private async handleSwipe(
    ...
    await swipe(startX, startY, endX, endY, undefined, this.deviceId);
    return { success: true, shouldFinish: false };
  }
  ...
  private async handleLongPress(
    ...
    const [x, y] = this.convertRelativeToAbsolute(element, width, height);
    await longPress(x, y, 3000, this.deviceId);
    return { success: true, shouldFinish: false };
  }
